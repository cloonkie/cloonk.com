// TypeScript source. Builds to dist/sellout-standardizer/sellout-standardizer.js.
...
function getCadenceLookup() {
  if (_cadenceLookup) return _cadenceLookup;
  const flat = [];
  CADENCE_VOCAB.forEach(entry => {
    entry.aliases.forEach(alias => {
      const tokens = normalize(alias).split(/\s+/).filter(Boolean);
      if (tokens.length) flat.push({ tokens, canonical: entry.canonical, yearBasis: entry.yearBasis || null });
    });
  });
...
function getCadenceStripRe() {
  if (_cadenceStripRe) return _cadenceStripRe;
  const raw = [];
  CADENCE_VOCAB.forEach(entry => entry.aliases.forEach(a => { if (a) raw.push(a); }));
...
function containsTokenSequence(haystack, needle) {
  if (!needle.length || !haystack.length || needle.length > haystack.length) return false;
...
function scoreAliasOverlap(headerNorm, aliasNorm) {
  ...
  if (headerNorm === aliasNorm) return 1.5;
  ...
  let matched = 0;
  for (const t of aliasTokens) if (headerSet.has(t)) matched++;
  if (!matched) return 0;
...
function bestAliasScore(headerLabel, aliases) {
  if (!headerLabel || !aliases || !aliases.length) return 0;
...
function getMetricOptions() {
  ...
  (BASE_CADENCE_METRIC_OPTIONS as any[]).concat(getCustomMetrics().map(metric => ({ ...metric, custom: true }))).forEach(metric => {
    if (!metric || DIMENSION_ONLY_KEYS.has(metric.key) || seenKeys.has(metric.key)) return;
    const normalized = metric.custom ? normalizeMetricDefinition(metric) : metric;
    if (!normalized) return;
    const labelKey = normalize(normalized.label);
    if (!metric.custom && seenLabels.has(labelKey)) return;
    if (metric.custom && options.some(existing => bestAliasScore(normalized.label, existing.aliases) >= 1)) return;
...
function getSheetIncluded(source) {
  ...
  if (bySheet !== undefined) return !!bySheet;
  return source.included !== false;
}

function getSourceOverride(source) {
  if (!state.config.sourceOverrides) return null;
...
function ensureSourceOverride(source) {
  ...
  if (!state.config.sourceOverrides[key].dimensions) state.config.sourceOverrides[key].dimensions = {};
  if (typeof state.config.sourceOverrides[key].active !== "boolean") state.config.sourceOverrides[key].active = false;
...
function suggestSourceOverrides(name, sheet) {
  ...
  const result: any = { active: false, yearBasis: "", dimensions: {} };
  if (/\b(ly|last\s*year|restated|prior\s*year)\b/i.test(text)) result.yearBasis = "LY";
...
function parseRowSpec(spec): Set<number> {
  ...
  String(spec || "").split(",").forEach(part => {
    ...
    if (Number.isInteger(row) && row > 0) rows.add(row);
  });
  return rows;
}
...
function columnLabelToIndex(label) {
  ...
  if (!/^[A-Z]+$/.test(raw)) return NaN;
  let index = 0;
  for (let i = 0; i < raw.length; i++) index = index * 26 + (raw.charCodeAt(i) - 64);
  return index - 1;
}
...
function parseColumnSpec(spec): Set<number> {
  ...
  String(spec || "").split(",").forEach(part => {
    ...
    if (range) {
      ...
      for (let col = Math.min(start, end); col <= Math.max(start, end); col++) cols.add(col);
      return;
    }
...
function getRowSkipReason(row, originalRow) {
  if (getDeletedRows().has(originalRow)) return "manual delete";
...
function getRowsForSource(source, options: any = {}) {
...
function toMetricName(label) {
  ...
  for (const metric of getMetricOptions()) {
    ...
    if (score > bestScore) { bestScore = score; bestKey = metric.key; }
  }
...
function hideLoadProgress() {
  if (els.loadProgress) els.loadProgress.classList.remove("show");
}
...
function addSource(name, sheet, rows, options: any = {}) {
...
async function loadPendingWorkbook(id) {
  ...
  if (!selected.length) { toast("Select at least one sheet first."); return; }
  try {
...
function autoDetect() {
  ...
  if (header) state.config.headerRow = header.idx + 1;
  ...
  scanRows.forEach(cell => {
    ...
    if (info) {
      ...
      if (info.yearBasis === "LY") detectedLy = true;
    }
    if (LY_LABEL_RE.test(text)) detectedLy = true;
  });
  if (detected.size) {
    const merged = new Set([...(state.config.cadenceTypes || []), ...detected]);
    state.config.cadenceTypes = CADENCES.filter(c => merged.has(c));
  }
  if (detectedLy) state.config.includeLy = true;
...
function buildHeaders() {
  ...
  const cadenceInfoByCol = Array.from({ length: maxCols }, (_, idx) => {
    ...
    if (cadenceRaw) {
      ...
      if (info) { inheritedCadenceInfo = info; return info; }
  ...
  state.headers = Array.from({ length: maxCols }, (_, idx) => {
    ...
    let yearBasis;
    if (LY_LABEL_RE.test(header)) yearBasis = "LY";
    else if (TY_LABEL_RE.test(header)) yearBasis = "TY";
    else if (cadenceRaw && LY_LABEL_RE.test(cadenceRaw)) yearBasis = "LY";
    else if (effectiveCadenceInfo && effectiveCadenceInfo.yearBasis) yearBasis = effectiveCadenceInfo.yearBasis;
    else yearBasis = "TY";
...
function autoMap() {
  ...
  state.headers.forEach(h => {
    if (takenDimCols.has(h.index) || takenMetricCols.has(h.index)) return;
    const stripped = stripMetaLabels(h.raw) || h.raw;
    dimFields.forEach(field => {
      const score = Math.max(
        bestAliasScore(stripped, field.aliases),
        bestAliasScore(h.raw, field.aliases)
      );
      if (score >= DIM_THRESHOLD) dimCandidates.push({ score, fieldKey: field.key, colIdx: h.index });
    });
  });
  dimCandidates.sort((a, b) => b.score - a.score);
  dimCandidates.forEach(cand => {
    if (isMappedColumn(dimMappings[cand.fieldKey])) return;
    if (dimMappings[cand.fieldKey] !== undefined && dimMappings[cand.fieldKey] !== null) return;
    if (takenDimCols.has(cand.colIdx) || takenMetricCols.has(cand.colIdx)) return;
  ...
  getActiveMappingTriples().forEach(({ cadence, yearBasis, metric }) => {
    ...
    if (metricMappings[key] !== undefined) return;
    state.headers.forEach(h => {
      if (takenDimCols.has(h.index) || takenMetricCols.has(h.index)) return;
      if (h.detectedCadence && h.detectedCadence !== cadence) return;
      if (h.detectedYearBasis && h.detectedYearBasis !== yearBasis) return;
      const stripped = stripMetaLabels(h.raw) || h.raw;
      const score = bestAliasScore(stripped, metric.aliases);
      if (score < METRIC_THRESHOLD) return;
  ...
  metricCandidates.forEach(cand => {
    if (metricMappings[cand.key] !== undefined && metricMappings[cand.key] !== null) return;
    if (takenMetricCols.has(cand.colIdx)) return;
...
function buildOutput() {
  ...
  getIncludedSources().forEach(source => {
    ...
    workingRows.slice(headerIdx + 1).forEach(item => {
      const row = item.row;
      if (!row.some(cell => cleanHeader(cell))) return;
      const dims = {};
      ...
      triples.forEach(({ cadence, yearBasis, metric }) => {
        ...
        if (!isMappedColumn(mapped)) return;
        const col = Number(mapped);
        const rawValue = row[col];
        if (rawValue === undefined || cleanHeader(rawValue) === "") return;
        if (state.config.skipZeroValues && parseNumber(rawValue) === 0) return;
        ...
        getActiveCadences().forEach(cadence => {
          getYearBases().forEach(yearBasis => {
            ...
            calculatedMetrics.forEach(metric => {
              ...
              if (!Number.isFinite(value)) return;
              if (state.config.skipZeroValues && value === 0) return;
              outputs.push({
                source_file: source.name,
                source_sheet: source.sheet,
                source_row: item.originalRow,
...
function renderFileList() {
  state.pendingWorkbooks = state.pendingWorkbooks || [];
  if (els.dropzone) els.dropzone.hidden = state.sources.length > 0 || state.pendingWorkbooks.length > 0;
  ...
  const sourceHtml = state.sources.map(source => {
    ...
    const dimChips = dimEntries.length ? dimEntries.map(([k, v]) =>
      `<span class="override-chip">${esc(k)} = ${esc(v)}<button type="button" data-remove-override-dim="${esc(source.id)}|${esc(k)}" aria-label="Remove ${esc(k)} override">&times;</button></span>`
    ).join("") : '<span class="override-empty">No dimension overrides set</span>';
...
function getCheckedValues(container, selector, datasetKey) {
  return (Array.from(container.querySelectorAll(selector)) as any[])
    .filter(input => input.checked)
    .map(input => input.dataset[datasetKey]);
}

function renderDeleteColumnChecklist() {
  if (!els.deleteColumnChecklist) return;
  const source = getIncludedSources()[0];
  if (!source) {
...
function renderHeaderDefinition() {
  if (!els.headerDefinition) return;
  const source = getIncludedSources()[0];
  if (!source) { els.headerDefinition.innerHTML = ""; return; }
...
function renderMappingSummary() {
  if (!els.mappingSummary) return;
  if (!state.headers.length) { els.mappingSummary.innerHTML = ""; return; }
  const triples = getActiveMappingTriples();
  const mappedDims = countMappedDimensions();
  const mappedMetrics = countMappedMetrics();
...
function renderMappings() {
  if (!state.headers.length) {
    ...
    if (els.mappingSummary) els.mappingSummary.innerHTML = "";
  ...
  if (!state.config.showUnmappedOnlyTouched) {
    ...
    if (total > 0 && mapped / total >= 0.5) {
      ...
      if (els.showUnmappedOnly) els.showUnmappedOnly.checked = true;
    }
  }
  const showUnmappedOnly = !!state.config.showUnmappedOnly;
  els.mappingGrid.classList.add("scroll-map");
  const options = ['<option value="">Unmapped</option>'].concat(
    state.headers.map(h => `<option value="${h.index}">${esc(headerOptionLabel(h))}</option>`)
  ).join("");
  ...
  const dimRows = groups.map(group => {
    ...
    if (showUnmappedOnly && mappedCount === group.keys.length) return "";
    return `
  ...
  const triples = getActiveMappingTriples();
  const visibleTriples = triples.filter(({ cadence, yearBasis, metric }) =>
    !showUnmappedOnly || !isMappedColumn((state.config.metricMappings || {})[metricMapKey(cadence, yearBasis, metric.key)])
  );
  ...
  getStandardFields().forEach(field => {
    ...
    if (select && isMappedColumn(state.config.mappings[field.key])) select.value = String(state.config.mappings[field.key]);
  });
  triples.forEach(({ cadence, yearBasis, metric }) => {
    const key = metricMapKey(cadence, yearBasis, metric.key);
    const id = `metricmap-${key.replace(/[^a-z0-9]+/gi, "-")}`;
...
function renderChips() {
  ...
  const chips = [];
  if (deletedRows) chips.push(`<span class="chip warn">${deletedRows} rows deleted</span>`);
  if (deletedCols) chips.push(`<span class="chip warn">${deletedCols} cols deleted</span>`);
  if (state.config.skipZeroValues) chips.push(`<span class="chip warn">zero skipped</span>`);
  if (state.config.includeLy) chips.push(`<span class="chip good">LY on</span>`);
  if (customDims) chips.push(`<span class="chip good">+${customDims} dim</span>`);
  if (customMetrics) chips.push(`<span class="chip good">+${customMetrics} metric</span>`);
  if (calcMetrics) chips.push(`<span class="chip good">${calcMetrics} calc</span>`);
...
function getActiveOutputMetrics() {
  ...
  if (!state.config.outputMetricsTouched && !selected.size) return new Set(options.map(item => item.value));
  return selected;
}
...
function getActiveOutputDimensions() {
  ...
  if (!state.config.outputDimensionsTouched && !selected.size) return new Set(options.map(item => item.value));
  return selected;
}
...
function getBreakoutAxisValue(row, axis) {
  if (axis === "year_basis") return row.year_basis || "TY";
  if (axis === "cadence") return row.cadence || "";
...
function getFilteredOutputRows() {
  ...
  const meta: any = state.config.outputMeta || {};
  ...
  if (breakoutAxes.length) {
    ...
    rows = Array.from(grouped.values()).map(row => Object.fromEntries(
    ...
    rows = baseRows.map(row => {
      ...
      if (meta.value_source !== false) filtered.value_source = row.value_source;
  ...
  const baseCols = rows.reduce((cols, row) => {
    Object.keys(row).forEach(key => { if (!cols.includes(key)) cols.push(key); });
    return cols;
  }, []);
...
function applyOutputColumnOrder(baseKeys, currentOrder) {
  ...
  if (!kept.length) return sortByCategory(baseKeys);
...
function applyColumnOrder(baseKeys, currentOrder) {
  ...
  baseKeys.forEach(key => { if (!present.has(key)) ordered.push(key); });
  return ordered;
}
...
function reorderColumnKeys(orderArr, fromKey, toKey) {
  ...
  if (fromIdx < 0 || toIdx < 0 || fromIdx === toIdx) return orderArr;
  const next = [...orderArr];
...
function getOutputColumnRole(col) {
  if (["source_file", "source_sheet", "source_row", "value_source"].includes(col)) return "metadata";
  if (["cadence", "year_basis", "metric", "value"].includes(col)) return "metric";
  if (getStandardFields().some(field => field.key === col)) return "dimension";
  return "metric";
}
...
function getExportColumnLabel(key) {
  ...
  if (field) return toTitleLabel(field.label);
  if (/[\s$%]/.test(key)) return cleanHeader(key);
...
function isOutputValueColumnKey(key) {
  ...
  if (["cadence", "year_basis", "metric"].includes(key)) return false;
...
function getExportNumericLabels(sourceRows = getFilteredOutputRows()) {
  ...
  if (sourceRows.length) {
    Object.keys(sourceRows[0]).forEach(key => {
      if (isOutputValueColumnKey(key)) labels.add(getExportColumnLabel(key));
    });
  }
  return labels;
}
...
function getHeaderAssignmentValue(headerIndex) {
  ...
  if (dimensionKey) return `dimension:${dimensionKey}`;
  const metricKey = getHeaderMetricKey(headerIndex);
  if (metricKey) return `metric:${metricKey}`;
  return "";
}
...
function assignHeaderColumn(headerIndex, value) {
  ...
  Object.entries(state.config.mappings).forEach(([key, idx]) => {
    if (Number(idx) === headerIndex) previousDimensionKeys.push(key);
  });
  state.config.metricMappings = state.config.metricMappings || {};
  Object.entries(state.config.metricMappings).forEach(([key, idx]) => {
    if (Number(idx) === headerIndex) previousMetricKeys.push(key);
  });
  ...
  const key = rest.join(":");
  if (type === "dimension") state.config.mappings[key] = headerIndex;
  if (type === "metric") state.config.metricMappings[key] = headerIndex;
  if (type === "custom-dimension") {
    const custom = addCustomFieldFromHeader(headerIndex, "dimension");
    if (custom) state.config.mappings[custom.key] = headerIndex;
  }
  if (type === "custom-metric") {
    const custom = addCustomFieldFromHeader(headerIndex, "metric");
    if (custom) state.config.metricMappings[metricMapKey(getActiveCadences()[0] || "WTD", "TY", custom.key)] = headerIndex;
  }
}
...
function adjustMappingsAfterColumnDelete(currentIndex) {
  Object.entries(state.config.mappings || {}).forEach(([key, value]) => {
    if (!isMappedColumn(value)) return;
    const idx = Number(value);
    if (idx === currentIndex) state.config.mappings[key] = null;
    else if (idx > currentIndex) state.config.mappings[key] = idx - 1;
  });
  Object.entries(state.config.metricMappings || {}).forEach(([key, value]) => {
    if (!isMappedColumn(value)) return;
    const idx = Number(value);
    if (idx === currentIndex) state.config.metricMappings[key] = null;
    else if (idx > currentIndex) state.config.metricMappings[key] = idx - 1;
  });
}
...
function renderPivotTable() {
  ...
  const rowDimSet = new Set<string>((state.config.pivotDimensions || []).filter(key => dimensionOptions.some(item => item.value === key)));
  const colDimSet = new Set<string>((state.config.pivotColumnDimensions || []).filter(key => dimensionOptions.some(item => item.value === key) && !rowDimSet.has(key)));
  ...
  state.output.forEach(row => {
    if (!activeMetricSet.has(row.metric)) return;
    const numeric = parseNumber(row.value);
    if (!Number.isFinite(numeric)) return;
    ...
    if (!colDimComboMap.has(colDimKey)) colDimComboMap.set(colDimKey, colDimValues);
    const dimValues = rowDims.map(key => cleanHeader(row[key]));
    const rowKey = [...dimValues, row.metric].join("\u001f");
    if (!rows.has(rowKey)) rows.set(rowKey, { metric: row.metric, dimensions: dimValues, total: 0, cells: {} });
  ...
  cadenceCols.sort((a, b) => {
    ...
    if (cadenceDelta) return cadenceDelta;
        ...
        ${sortedRows.map(row => `
          <tr>
            ${orderedCols.map(col => {
              ...
              if (col.kind === "metric") return `<td class="${col.role}-col" title="${esc(row.metric)}">${esc(getMetricLabel(row.metric))}</td>`;
              if (col.kind === "cell") return `<td class="${col.role}-col">${esc(formatPivotValue(row.cells[col.cellKey] || 0))}</td>`;
...
function getRawColumnLabels(rows) {
  ...
  for (let original = 0; originalIndexes.length < maxCols; original++) {
    if (!deletedColumns.has(original)) originalIndexes.push(original);
  }
...
function renderAssignPopover(headerIndex, anchor) {
  ...
  const optionStateClass = value => {
    ...
    if (assignedIndex === headerIndex || value === currentValue) return " is-current";
    return " is-assigned";
  };
...
function selectOptions(items, currentValue, emptyLabel) {
  return [`<option value="">${esc(emptyLabel)}</option>`].concat(
    items.map(item => `<option value="${esc(item.value)}" ${String(currentValue || "") === String(item.value) ? "selected" : ""}>${esc(item.label)}</option>`)
  ).join("");
}
...
function buildStakeholderSummaryRows() {
  ...
  return (kpiRows as any[]).concat([{}], topSkuRows, [{}], topDoorRows);
}
...
function clearPivotTemplate() {
  ...
  pivotTemplateName = "";
  try { localStorage.removeItem(PIVOT_TEMPLATE_STORAGE_KEY); } catch (e) {}
  renderPivotTemplateStatus();
  toast("Pivot template cleared.");
}

function renderPivotTemplateStatus() {
  if (!els.pivotTemplateStatus) return;
...
async function exportPivotWorkbook() {
  if (!state.output.length) return;
  if (!window.XLSX) { toast("XLSX library unavailable."); return; }
  if (!window.JSZip) { toast("JSZip library unavailable."); return; }
...
function saveConfigToHistory(name, config?: any) {
...
function renderConfigHistory() {
  if (!els.configHistoryList) return;
...
function applySavedConfig(id) {
  ...
  if (!item) { toast("Saved configuration not found."); return; }
...
function hydrateConfig(config) {
  ...
  state.config.customMetrics.forEach(rawMetric => {
    ...
    if (!metric || DIMENSION_ONLY_KEYS.has(metric.key)) return;
    const baseMatch = BASE_CADENCE_METRIC_OPTIONS.find(baseMetric =>
      baseMetric.key === slugifyKey(metric.label) || bestAliasScore(metric.label, baseMetric.aliases) >= 1
    );
    if (baseMatch) {
    ...
    if (nextKey !== metric.key) metricKeyRenames.set(metric.key, nextKey);
  ...
  if (state.config.metricMappings && typeof state.config.metricMappings === "object") {
    Object.entries({ ...state.config.metricMappings }).forEach(([key, value]) => {
      ...
      if (renamed === parts[2]) return;
      const nextKey = metricMapKey(parts[0], parts[1], renamed);
      if (state.config.metricMappings[nextKey] === undefined) state.config.metricMappings[nextKey] = value;
  ...
  Object.keys(state.config.metricMappings).forEach(key => {
    ...
    if (DIMENSION_ONLY_KEYS.has(metricKey)) delete state.config.metricMappings[key];
  });
  ...
  if (els.showUnmappedOnly) els.showUnmappedOnly.checked = !!state.config.showUnmappedOnly;
}

function addCustomDimension(label) {
  const baseKey = slugifyKey(label);
  if (!label || !baseKey) { toast("Enter a dimension name first."); return false; }
...
function addCustomMetric(label) {
  ...
  const baseKey = slugifyKey(cleanLabel);
  if (!cleanLabel || !baseKey) { toast("Enter a metric name first."); return false; }
...
function addCustomCalculatedMetric(label, formula) {
  const baseKey = slugifyKey(label);
  if (!label || !baseKey || !formula) { toast("Provide both a name and a formula."); return false; }
...
function toast(message) {
  ...
  (toast as any).timer = setTimeout(() => els.toast.classList.remove("show"), 3200);
}
...
function renderChipOptions(items, selectedValues, dataAttr, emptyText) {
  if (!items.length) return `<span class="chip">${esc(emptyText)}</span>`;
...
els.fileModeBtn.addEventListener("click", () => {
  els.fileModeBtn.classList.add("active"); els.pasteModeBtn.classList.remove("active");
  els.fileMode.style.display = ""; els.pasteMode.style.display = "none";
});
els.pasteModeBtn.addEventListener("click", () => {
  els.pasteModeBtn.classList.add("active"); els.fileModeBtn.classList.remove("active");
  els.pasteMode.style.display = ""; els.fileMode.style.display = "none";
});
els.addPasteBtn.addEventListener("click", () => {
  const rows = parseDelimited(els.pasteBox.value);
  if (!rows.length) return toast("Paste a copied Excel range first.");
...
els.fileList.addEventListener("click", e => {
  ...
  if (addBtn) {
    ...
    if (!field || !value) { toast("Pick a dimension and enter a value."); return; }
...
els.fileList.addEventListener("change", e => {
  ...
  if (pendingSheet) {
    ...
    if (!workbook) return;
    const selected = new Set(workbook.selectedSheets || []);
    if (pendingSheet.checked) selected.add(pendingSheet.value);
    else selected.delete(pendingSheet.value);
...
els.deleteRows.addEventListener("click", () => {
  ...
  if (field) field.classList.toggle("open", isOpen);
});
...
els.cadenceTypeSelect.addEventListener("change", e => {
  ...
  if (input.checked) set.add(input.dataset.cadenceType);
  else set.delete(input.dataset.cadenceType);
...
els.saveCustomCalcBtn.addEventListener("click", () => {
  ...
  const baseKey = slugifyKey(label);
  if (!label || !baseKey || !formula) return toast("Name the calculated metric and enter a formula.");
...
els.addFieldSaveBtn.addEventListener("click", () => {
  ...
  let ok = false;
  if (type === "dimension") ok = addCustomDimension(label);
  else if (type === "metric") ok = addCustomMetric(label);
  else if (type === "calculated") ok = addCustomCalculatedMetric(label, formula);
  if (ok) closeAddFieldPopover();
});
els.addFieldName.addEventListener("keydown", e => { if (e.key === "Enter") els.addFieldSaveBtn.click(); });
els.addFieldFormula.addEventListener("keydown", e => { if (e.key === "Enter") els.addFieldSaveBtn.click(); });
...
els.mappingGrid.addEventListener("change", e => {
  ...
  if (dimSelect) {
    if (dimSelect.value === "") state.config.mappings[dimSelect.dataset.map] = null;
    else state.config.mappings[dimSelect.dataset.map] = Number(dimSelect.value);
    rebuild();
    return;
  }
  ...
  if (metricSelect) {
    state.config.metricMappings = state.config.metricMappings || {};
    if (metricSelect.value === "") state.config.metricMappings[metricSelect.dataset.metricMap] = null;
    else state.config.metricMappings[metricSelect.dataset.metricMap] = Number(metricSelect.value);
    rebuild();
  }
});
...
els.tableShell.addEventListener("dragover", e => {
  ...
  els.tableShell.querySelectorAll("th.col-drag-over").forEach(el => { if (el !== th) el.classList.remove("col-drag-over"); });
...
els.tableShell.addEventListener("drop", e => {
  ...
  if (fromView !== toView || fromKey === toKey) return;
  if (toView === "output") {
...
els.tableShell.addEventListener("contextmenu", async e => {
  ...
  if (rowTarget) {
    ...
    if (action !== "d" && action !== "delete") return;
    if (!await confirmDelete(`Delete row ${rowNumber} from this import?`)) return;
  ...
  if (colTarget) {
    ...
    if (!await confirmDelete(`Delete column ${label} from this import?`)) return;
...
els.assignPopover.addEventListener("click", e => {
  ...
  if (custom) {
    ...
    if (!name) return toast("Name the custom field first.");
    const field = addCustomFieldWithLabel(name, custom.dataset.customAssign);
    if (!field) return;
    if (custom.dataset.customAssign === "dimension") assignHeaderColumn(headerIndex, `dimension:${field.key}`);
    else assignHeaderColumn(headerIndex, `metric:${metricMapKey(getActiveCadences()[0] || "WTD", "TY", field.key)}`);
    closeAssignPopover();
    rebuild();
  }
});
document.addEventListener("click", e => {
  if (!els.assignPopover?.classList.contains("open")) return;
  if (e.target.closest("#assignPopover") || e.target.closest("[data-open-assign]")) return;
  closeAssignPopover();
});
els.tableShell.addEventListener("change", e => {
  ...
  if (pivotDimension) {
    ...
    if (pivotDimension.checked) set.add(key);
    else set.delete(key);
  ...
  if (pivotColDim) {
    ...
    if (pivotColDim.checked) set.add(key);
    else set.delete(key);
  ...
  if (pivotMetric) {
    ...
    if (pivotMetric.checked) set.add(pivotMetric.dataset.pivotMetric);
    else set.delete(pivotMetric.dataset.pivotMetric);
...
els.outputControls.addEventListener("change", e => {
  ...
  if (dim) {
    ...
    if (dim.checked) set.add(dim.dataset.outputDimension);
    else set.delete(dim.dataset.outputDimension);
  ...
  if (metric) {
    ...
    if (metric.checked) set.add(metric.dataset.outputMetric);
    else set.delete(metric.dataset.outputMetric);
  ...
  if (breakout) {
    ...
    if (breakout.checked) set.add(breakout.dataset.outputBreakout);
    else set.delete(breakout.dataset.outputBreakout);
  ...
  if (breakoutValue) {
    ...
    if (breakoutValue.checked) set.add(breakoutValue.dataset.outputBreakoutValue);
    else set.delete(breakoutValue.dataset.outputBreakoutValue);
...
els.pivotTemplateInput.addEventListener("change", async e => {
  ...
  if (file) await setPivotTemplate(file);
  e.target.value = "";
});
...
els.configMenu.addEventListener("click", e => {
  ...
  if (loadBtn) applySavedConfig(loadBtn.dataset.loadSavedConfig);
  if (deleteBtn) deleteSavedConfig(deleteBtn.dataset.deleteSavedConfig);
});
document.addEventListener("click", e => {
