...
function hasMeaningfulDataKeyState(st){
  if(!st) return false;
  if(normalizeStatus(st.status)!=='na') return true;
  if(normalizeGender(st.gender)!=='ALL') return true;
  if(!isBlankSnapshotValue(st.note)) return true;
  if(!isBlankSnapshotValue(st.metric_1)) return true;
  if(!isBlankSnapshotValue(st.metric_2)) return true;
...
function compactDataKeyStateForSnapshot(){
  const compact:any={};
  Object.entries<any>(dataKeyState||{}).forEach(([key,st])=>{
    if(!hasMeaningfulDataKeyState(st)) return;
...
function getMaxDoorsForRetailer(ret){
  ...
  for(const d of doorLocations){
    ...
    if(d.retired || !isPhysicalDoorNumber(d.doorNumber)) continue;
...
function updateStoreDoorField(ret,doorNumber,field,el){
  ...
  if(field==='iss' || field==='three_c'){
    ...
    const duplicate=doorLocations.find(x=>
      x!==d &&
...
function remapDoorNumberKeys(ret,oldNum,newNum){
  ...
  Object.keys(dataKeyState).forEach(key=>{
    const st=dataKeyState[key];
    if(!st || normalizeRetailer(st.retailer)!==norm) return;
  ...
  Object.keys(doorAssignments).forEach(ak=>{
    ...
    (doorAssignments[ak]||[]).forEach(a=>{
      if(String(a.doorNumber)===String(oldNum)){
        a.doorNumber=newNum;
        if(door) a.doorName=door.name||`Door ${newNum}`;
      }
    });
  });
}
...
function syncAssignmentDoorName(ret,doorNumber,doorName){
  ...
  Object.entries<any>(doorAssignments||{}).forEach(([ak,assigns])=>{
    ...
    if(normalizeRetailer(r)!==norm) return;
    (assigns||[]).forEach(a=>{
      if(String(a.doorNumber)===String(doorNumber)) a.doorName=nextName;
    });
  });
}
...
function renderDoorDetail(d,ret){
  ...
  if(!_marketDataPromise){
    loadMarketData().then(()=>{ if(_doorDetailCurrent===d) renderDoorDetail(d,ret); }).catch(()=>{});
  }
...
function fetchJsonQuiet(url,opts?:RequestInit){
...
function rebuildDoorTradeAreaIndex(){
  ...
  (_doorTradeAreaData.features||[]).forEach(f=>{
    ...
    if(!_doorTradeAreaIndex.has(key)) _doorTradeAreaIndex.set(key,p);
  });
}
...
function updateDataKeyField(ret,doorNumber,brand,field,value,el?){
  ...
  if(el && field==='status') syncDataCellClass(el,field);
