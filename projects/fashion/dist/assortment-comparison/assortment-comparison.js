// TypeScript source. Builds to dist/assortment-comparison/assortment-comparison.js.
...
function loadRows(rows, options = {}) {
    rawRows = rows;
...
function getFilteredRows(selectedRetailers) {
    ...
    return normalizedRows.filter(row => {
        if (selectedRetailers.length && !selectedRetailers.includes(row.retailer))
            return false;
        if (brandSet && !brandSet.has(row.brand))
            return false;
        if (search) {
...
function buildProductMap(rows) {
    ...
    rows.forEach(row => {
        ...
        item.oh += row.unitOh;
        if (!item.release && row.release)
            item.release = row.release;
        if (!item.upc && row.upc)
            item.upc = row.upc;
        if (!item.style && row.style)
...
function buildRegionLabels(retailers, segmentByKey, positions) {
    const combos = Object.values(segmentByKey);
    ...
    for (let xi = 0; xi <= steps; xi++) {
        ...
        for (let yi = 0; yi <= steps; yi++) {
            ...
            for (const i of includedIdx) {
                ...
                if (clearance < minIncludedClearance)
                    minIncludedClearance = clearance;
            }
            if (!ok)
                continue;
            ...
            for (const i of excludedIdx) {
                ...
                if (clearance < minExcludedClearance)
                    minExcludedClearance = clearance;
            }
            if (!ok)
                continue;
...
function restoreSnapshot() {
    withStore('readonly', store => store.get(SAVE_KEY)).then((snapshot) => {
        if (!snapshot || !snapshot.rawRows || !snapshot.rawRows.length)
            return;
        hydrating = true;
