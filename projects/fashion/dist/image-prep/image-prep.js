// TypeScript source. Builds to dist/image-prep/image-prep.js.
...
(function () {
    ...
    function addFiles(fileList) {
        ...
        if (!incoming.length) {
            toast("No images found in that drop.");
            return;
        }
        ...
        incoming.forEach((file) => {
            ...
            const item = { id, file, name: file.name, url, img: null, w: 0, h: 0, ok: false, error: "" };
            items.push(item);
            const img = new Image();
            img.onload = () => {
    ...
    function computeTarget(nw, nh) {
        ...
        if (s.resize === "exact") {
            ...
            if (s.exFit === "stretch")
                return { cw, ch, dw: cw, dh: ch, dx: 0, dy: 0 };
    ...
    function effectiveBg(type) {
        ...
        if (type === "image/jpeg")
            return "#ffffff"; // JPEG can't be transparent
    ...
    function showResizePanels(mode) {
        document.querySelectorAll("[data-rz]").forEach((el) => { el.hidden = el.dataset.rz !== mode; });
    }
    function showBgPanels(mode) {
        document.querySelectorAll("[data-bg]").forEach((el) => { el.hidden = el.dataset.bg !== mode; });
        updateJpegNote();
    }
    function showFmtPanels(fmt) {
        const q = document.querySelector('[data-fmt="quality"]');
        if (q)
            q.hidden = !(fmt === "image/jpeg" || fmt === "image/webp");
        updateJpegNote();
    }
    ...
    function initSettings() {
        ...
        $("pctRange").addEventListener("input", (e) => {
            settings.percent = +e.target.value;
        ...
        const setColor = (v) => {
            ...
            color.value = settings.bgColor;
            hex.value = settings.bgColor;
            syncSwatch();
            scheduleProcess();
        };
        ...
        // Preset swatches
        $("presetSwatches").innerHTML = PRESET_COLORS.map((c) => `<button type="button" data-color="${c}" style="background:${c}" title="${c}" aria-label="${c}"></button>`).join("");
    ...
    function initInfoModal() {
        ...
        document.addEventListener("keydown", (e) => { if (e.key === "Escape")
            close(); });
        document.querySelectorAll(".info-modal-tab").forEach((tab) => tab.addEventListener("click", () => activate(tab.dataset.tab)));
    }
    /* ── Wiring ─────────────────────────────────────────────── */
    function init() {
        // Intake
        ...
        ["dragleave", "dragend", "drop"].forEach((ev) => els.dropzone.addEventListener(ev, () => els.dropzone.classList.remove("dragging")));
        ...
        els.queue.addEventListener("click", (e) => {
            ...
            if (rm)
                removeItem(+rm.dataset.remove);
        });
        els.resultsGrid.addEventListener("click", (e) => {
            const dl = e.target.closest("[data-dl]");
            if (dl)
                download(items.find((it) => it.id === +dl.dataset.dl));
        });
