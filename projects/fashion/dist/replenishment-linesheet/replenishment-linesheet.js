// TypeScript source. Builds to dist/replenishment-linesheet/replenishment-linesheet.js.
...
function handleImageFiles(e) {
    ...
    imageFiles.forEach(file => {
        ...
        const item = allItems.find(it => it.sku.toLowerCase() === nameNoExt ||
        ...
        if (firstSheet && sheetRels) {
            ...
            if (drawingRel) {
                ...
                if (drawingFile && drawingRelsFile) {
                    ...
                    const anchors = Array.from(drawingDoc.getElementsByTagName('*'))
                    ...
                    for (const anchor of anchors) {
                        ...
                        if (jsonRowIndex < 0 || images[jsonRowIndex])
                            continue;
...
async function extractRichDataImages(zip, firstSheetPath, images) {
    ...
    const firstSheet = zip.file(firstSheetPath);
    if (!richValuesFile || !webImagesFile || !webImagesRelsFile)
        return;
    ...
    const richValues = Array.from(richValuesDoc.getElementsByTagName('*')).filter(el => el.localName === 'rv');
    const webImages = Array.from(webImagesDoc.getElementsByTagName('*')).filter(el => el.localName === 'webImageSrd');
    const richValueImages = [];
    for (let rvIndex = 0; rvIndex < richValues.length; rvIndex++) {
        const values = Array.from(richValues[rvIndex].children).filter(el => el.localName === 'v').map(el => el.textContent || '');
        ...
        if (upcText)
            images.byUpc[upcText] = dataUrl;
    }
    if (!firstSheet)
        return;
    const sheetDoc = parseXml(await firstSheet.async('string'));
    Array.from(sheetDoc.getElementsByTagName('*')).filter(el => el.localName === 'c' && el.hasAttribute('vm')).forEach(cell => {
        ...
        if (rowNumber > 1 && dataUrl)
            images[rowNumber - 2] = dataUrl;
    });
}
...
function firstByLocalName(root, name) {
    if (!root)
        return null;
    return Array.from(root.getElementsByTagName('*')).find(el => el.localName === name);
}
...
function findRelationship(doc, id) {
    if (!id)
        return null;
    return Array.from(doc.getElementsByTagName('*')).find(el => el.localName === 'Relationship' && el.getAttribute('Id') === id);
}
...
function loadData(json, embeddedImages = {}, options = {}) {
    ...
    if (!options.notesMap)
        allItems.forEach(it => { if (it.notes)
    ...
    if (options.filters)
        applyFilterState(options.filters);
    // Show UI
...
function restoreAutosave() {
    readAutosaveSnapshot().then((snapshot) => {
        autosaveReady = true;
        if (!snapshot || !snapshot.allItems || !snapshot.allItems.length)
            return;
...
function scheduleAutosave() {
    if (!autosaveReady || autosaveHydrating || !allItems.length)
        return;
...
function withAutosaveStore(mode, callback) {
...
function normalizeImageSrc(v) {
    ...
    if (!s)
        return '';
    if (/^#(VALUE|N\/A|REF|NAME|NULL|DIV\/0|NUM)!?$/i.test(s))
        return '';
...
function getFiltered() {
    ...
    if (polarized)
        items = items.filter(it => String(it.polarized) === polarized);
    if (sel === 'selected')
        items = items.filter(it => selectedSet.has(it.id));
    if (sel === 'unselected')
        items = items.filter(it => !selectedSet.has(it.id));
    if (search)
        items = items.filter(it => it.style.toLowerCase().includes(search) ||
...
function toggleSelect(e, id) {
    ...
    if (cardEl) {
        ...
        if (typeof window.refreshCloonkCursorLabel === 'function')
            window.refreshCloonkCursorLabel(cardEl);
...
function renderPresentationItem(item) {
    ...
    notes.oninput = () => {
        ...
        if (cardNotes && cardNotes.value !== notes.value)
            cardNotes.value = notes.value;
        scheduleAutosave();
    };
...
function getPresentationSpecs(item) {
    ...
    if (item.custom1Label)
        specs.push([item.custom1Label, item.custom1, true]);
    if (item.custom2Label)
        specs.push([item.custom2Label, item.custom2, true]);
...
function closeLightbox(e) {
    ...
    if (e && e.target.closest && e.target.closest('.presentation-shell'))
        return;
    const lb = document.getElementById('lightbox');
...
(function setupInfoModal() {
    ...
    function showTab(name) {
        document.querySelectorAll('.info-modal-tab').forEach(t => t.classList.toggle('active', t.dataset.tab === name));
        document.querySelectorAll('.info-modal-pane').forEach(p => p.classList.toggle('active', p.id === 'modal-tab-' + name));
    }
...
function exportSelects() {
    ...
    if (!selected.length) {
        showToast('Select at least one SKU to export.');
        return;
    }
    const rows = selected.map(it => {
        ...
        if (it.custom1Label)
            row[it.custom1Label] = it.custom1;
        if (it.custom2Label)
            row[it.custom2Label] = it.custom2;
