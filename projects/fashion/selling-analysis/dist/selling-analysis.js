/* ============================================================================
   ...
   TypeScript source. Builds to dist/selling-analysis.js.
...
(function (root, factory) {
    ...
    function buildColumnMap(aoa) {
        ...
        for (let i = 0; i < Math.min(aoa.length, 25); i++) {
            ...
            if (score > best.score)
                best = { headerRow: i, map, headers, headerCandidates: candidates, score };
        }
    ...
    function enrich(records, params, themes) {
        ...
        return records.map(r => {
            ...
            e._flags = [];
            if (msrp == null || msrp <= 0)
                e._flags.push('msrp_invalid');
            if (u != null && u < 0)
                e._flags.push('neg_units');
            if (rtl != null && rtl < 0)
                e._flags.push('neg_retail');
            if (oh != null && oh < 0)
                e._flags.push('neg_oh');
            if (e.tyST != null && (e.tyST > 1 || e.tyST < 0))
                e._flags.push('st_out_of_range');
            if (e.aurRatio != null && e.aurRatio > 1.0)
                e._flags.push('aur_above_msrp');
            ...
            if (e.theme == null && e.collection != null && th[e.collection])
                e.theme = th[e.collection];
            return e;
        });
    }
    /* ---- generic helpers --------------------------------------------------- */
        ...
        return rows.filter(r => {
            for (const field in f) {
                if (field === 'relYear') {
                    ...
                    if (min != null && (r.relYear == null || r.relYear < min))
                        return false;
                    if (max != null && (r.relYear == null || r.relYear > max))
                        return false;
                    continue;
                }
    ...
    function velocityMatrix(rows) {
        ...
        const classify = r => {
            ...
            if (hiST && hiVol)
                return 'Star';
            if (hiST && !hiVol)
                return 'Sleeper'; // sells fast, low volume -> chase units
            if (!hiST && hiVol)
                return 'Slow-bleeder'; // big inventory, soft sell-through
            return 'Dog';
        };
    ...
    function linreg(pts) {
        const n = pts.length;
        if (n < 2)
            return { slope: 0, intercept: n ? pts[0][1] : 0 };
    ...
    function markdownSensitivityAnalysis(rows, nb) {
        ...
        const buckets = [];
        for (let i = 0; i < nb; i++)
            buckets.push({ lo: i / nb, hi: (i + 1) / nb, units: 0, rows: 0, stSum: 0, stN: 0 });
        valid.forEach(r => {
            const d = Math.max(0, Math.min(0.999, r.promoIndex));
            const b = buckets[Math.floor(d * nb)];
            b.units += r.tyUnits > 0 ? r.tyUnits : 0;
            b.rows++;
            if (r.tyST != null) {
                b.stSum += r.tyST;
                b.stN++;
    ...
    function demandForecast(rows, horizon) {
        ...
        for (let i = 0; i < horizon; i++) {
            ...
            proj.push({ step: i + 1, units: Math.max(0, fit.slope * x + fit.intercept) });
        }
    ...
    function collectionLifecycle(rows) {
        ...
        g.forEach((rs, k) => {
            ...
            if (ly === 0 && units > 0)
                stage = 'Launch';
            else if (units === 0 && oh > 0)
                stage = 'Exit';
            else if (ly > 0 && units < ly * 0.7)
                stage = 'Decay';
            else if (units > ly)
                stage = 'Growth';
    ...
    function coPerformance(rows, opts) {
        ...
        const groups = groupBy(rows.filter(r => r[by] != null && r[by] !== '' && r[item] != null && r[item] !== ''), r => r[by]);
    ...
    function cannibalization(rows) {
        ...
        g.forEach((rs, k) => {
            ...
            if (newGain > 0 && oldLoss > 0)
                out.push({ group: k, newGain, oldLoss, net: newGain - oldLoss, risk: Math.min(1, oldLoss / (newGain + oldLoss)) });
        });
    ...
    function whitespace(rows, opts) {
        ...
        const cell = {};
        rows.forEach(r => {
            if (r.retailer == null || r[attr] == null)
                return;
        ...
        attrs.forEach((a, ai) => retailers.forEach((ret, ri) => {
            if (rowTotals[ai] > 0 && matrix[ai][ri] === 0)
                gaps.push({ attr: a, retailer: ret, rowStrength: rowTotals[ai] });
        }));
    ...
    function invMatrix(A) {
        ...
        for (let col = 0; col < n; col++) {
            ...
            if (best < 1e-12)
                continue; // singular pivot → skip (pseudo-inverse)
            if (piv !== col) {
                const t = M[piv];
                M[piv] = M[col];
                M[col] = t;
    ...
    function ols(X, y) {
        ...
        if (!n || !p)
            return { beta: [], se: [], t: [], r2: null, adjR2: null, n, p };
        ...
        for (let a = 0; a < p; a++)
            for (let b = 0; b < a; b++)
                XtX[a][b] = XtX[b][a]; // symmetrize
        for (let a = 0; a < p; a++)
            XtX[a][a] += 1e-6; // tiny ridge for numerical stability
    ...
    function attributeDrivers(rows, opts) {
        ...
        if (dims.includes('frameMaterial') && dims.includes('materialCode'))
            dims = dims.filter(d => d !== 'materialCode');
        const target = useST ? 'st' : 'units';
        if (!dims.length || valid.length < 10)
            return { insufficient: true, terms: [], r2: null, n: valid.length, dims, target };
        const minCount = Math.max(3, Math.round(valid.length * 0.02));
        const baselines = {};
        let cols = []; // {dim, level, count} → one dummy column each
        dims.forEach(d => {
            const counts = {};
            valid.forEach(r => { const v = r[d]; if (v != null && v !== '')
                counts[v] = (counts[v] || 0) + 1; });
            const ls = Object.keys(counts).sort((a, b) => counts[b] - counts[a]);
            baselines[d] = ls[0]; // most common level is the reference
            ls.slice(1).forEach(l => { if (counts[l] >= minCount)
                cols.push({ dim: d, level: l, count: counts[l] }); });
        });
        ...
        if (cols.length > maxCols)
            cols = cols.slice().sort((a, b) => b.count - a.count).slice(0, maxCols);
        const p = cols.length + 1;
        const X = [], y = [];
        valid.forEach(r => {
            const row = new Array(p).fill(0);
            row[0] = 1; // intercept
            cols.forEach((c, j) => { if (String(r[c.dim]) === String(c.level))
                row[j + 1] = 1; });
            X.push(row);
    ...
    function seasonality(rows) {
        ...
        rows.forEach(r => {
            ...
            if (ym)
                dated.push({ month: ym.m, year: ym.y, ord: ym.y * 12 + ym.m, units: (r.tyUnits > 0 ? r.tyUnits : 0) });
        });
        if (dated.length < 6)
            return { insufficient: true, months: [] };
        const byMonth = {};
    ...
    function priceElasticity(rows) {
        ...
        if (valid.length < 8)
            return { insufficient: true, n: valid.length, points: [] };
