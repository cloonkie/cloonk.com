// TypeScript source. Builds to dist/upc-concat.js.
...
App.FormulaEngine = (function () {
    ...
    function tokenize(src) {
        ...
        while (i < n) {
            ...
            if (c === "{") {
                ...
                if (end < 0)
                    throw new Error("Unclosed { in formula");
    ...
    function parse(toks) {
        ...
        const ast = parseExpr();
        if (p < toks.length)
            throw new Error("Unexpected trailing token '" + toks[p].v + "'");
        return ast;
    }
    ...
    function fieldRefs(src) {
        const refs = new Set();
        try {
            tokenize(src || "").forEach(t => { if (t.t === "field")
                refs.add(t.v); });
        }
        catch (e) { /* ignore */ }
        return [...refs];
    ...
    function evaluate(ast, ctx, warnings) {
        ...
        switch (ast.k) {
            ...
            case "field": {
                if (!(ast.v in ctx)) {
                    if (warnings)
                        warnings.push("formula references undefined field {" + ast.v + "}");
                    return null;
                }
                return ctx[ast.v];
            }
            ...
            case "call": {
                ...
                if (!fn) {
                    if (warnings)
                        warnings.push("unknown function " + ast.fn);
                    return null;
                }
...
App.FileRegistry = (function () {
    ...
    function resolveColumn(file, ref) {
        ...
        if (r in file.colByName)
            return file.colByName[r];
        if (/^[A-Za-z]+$/.test(r)) {
...
App.ClusterEngine = {
    ...
    localIdentityValues(fileId, rowIdx) {
        ...
        schema.filter(f => f.type === "direct").forEach(f => {
            ...
            if (src) {
                ...
                if (ci != null)
                    ctx[f.name] = file.rows[rowIdx][ci];
            }
            if (!(f.name in ctx))
                ctx[f.name] = null;
        });
        ...
        App.State.resolveOrder.forEach(f => {
            if (f._ast)
                ctx[f.name] = App.FormulaEngine.evaluate(f._ast, ctx, null);
            else
                ctx[f.name] = null;
        });
        const out = [];
        schema.filter(f => f.is_identity).forEach(f => {
            const v = ctx[f.name];
            if (!U.isBlank(v))
                out.push({ field: f.name, value: U.normStr(v) });
        });
        return out;
    },
    async build(progressCb) {
        ...
        for (const fileId of fileIds) {
            ...
            for (let start = 0; start < n; start += 4000) {
                ...
                if (progressCb)
                    progressCb(done / Math.max(1, totalRows), "Clustering " + done.toLocaleString() + " / " + totalRows.toLocaleString());
    ...
    addRow(fileId, rowIdx) {
        ...
        allVariants.forEach(v => { if (S.aliasMap.has(v))
            hitIds.add(S.aliasMap.get(v)); });
        let cluster;
        if (hitIds.size === 0) {
    ...
    detectCollisions() {
        ...
        idFields.forEach(f => {
                ...
                fileIds.forEach(fid => {
                    c.rows[fid].forEach(ri => {
                        ...
                        U.variants(iv.value).forEach(v => {
                            if (!valToClusters.has(v))
                                valToClusters.set(v, new Set());
            ...
            let collisions = 0;
            valToClusters.forEach(set => { if (set.size > 1)
                collisions++; });
            if (collisions > 0) {
                S.warnings.push({
                    type: "warn",
...
App.Profile = {
    ...
    storedProfiles() {
        try {
            return JSON.parse(localStorage.getItem("upc_concat_profiles") || "{}");
        }
        catch (e) {
            return {};
        }
...
App.Exporter = {
    ...
    toXLSX() {
        if (!window.XLSX) {
            App.UI.toast("XLSX library unavailable — using CSV.");
            return this.csvFallback();
        }
        const rows = this.flatRows();
...
App.UI = {
    ...
    renderWarnings() {
        ...
        if (App.State._cycles && App.State._cycles.length) {
            App.State._cycles.forEach(c => items.unshift({ type: "err", msg: "Circular formula reference: " + c.join(" → ") }));
        }
        el.innerHTML = items.map(w => '<div class="warn-item ' + (w.type === "err" ? "err" : "") + '">' + U.esc(w.msg) + '</div>').join("");
    },
    /* ── 1. Sources ── */
    Sources: {
        render() {
            ...
            p.files.forEach(f => { if (!p.file_priority.includes(f.id))
                p.file_priority.push(f.id); });
        ...
        slotHTML(fd, rank) {
            ...
            if (loaded) {
                const missing = (fd.key_columns || []).filter(c => App.FileRegistry.resolveColumn(loaded, c) == null);
            ...
            if (loaded) {
                ...
                detailHTML =
                    '<div class="hint">Drop a file' +
                        ...
                        '<div class="keycols">' + (fd.key_columns || []).map(c => '<span class="keycol-chip">' + U.esc(c) + '</span>').join("") + '</div>';
            }
            ...
            try {
                ...
                toks.forEach((t, k) => {
                    if (k > 0)
                        out += " ";
                    if (t.t === "field")
                        out += '<span class="tok-field">{' + U.esc(t.v) + '}</span>';
                    else if (t.t === "str")
                        out += '<span class="tok-str">"' + U.esc(t.v) + '"</span>';
                    else if (t.t === "num")
                        out += '<span class="tok-num">' + t.v + '</span>';
                    else if (t.t === "ident") {
    ...
    Input: {
        ...
        updateDetect() {
            ...
            el.innerHTML = Object.keys(counts).map(d => '<span class="badge ' + (d === "unmatched" ? "warn" : "idn") + '">' +
    ...
    Results: {
        render() {
            ...
            document.getElementById("resultsSub").textContent = results.length
                ...
                : "No run yet";
            ["copyTsvBtn", "dlCsvBtn", "dlXlsxBtn"].forEach(id => document.getElementById(id).disabled = !results.length);
            ...
            rows = rows.filter(({ r }) => {
                ...
                if (blanksOnly && !hasBlank)
                    return false;
                if (resolvedOnly && hasBlank)
                    return false;
                if (filter) {
            ...
            function cellHTML(col, r, i) {
                if (col === "input_key")
                    return '<td title="' + U.esc(r.input_key) + '">' + U.esc(r.input_key) + '</td>';
                if (col === "detected_identity")
                    return '<td>' + (r.matched ? U.esc(r.detected_identity) : '<span class="dim">unmatched</span>') + '</td>';
                if (col === "cluster_id")
                    return '<td>' + (r.cluster_id != null ? r.cluster_id : "—") + '</td>';
                if (col === "trail")
                    return '<td><button class="trail-btn" data-trail="' + i + '">trail ▾</button></td>';
...
App.Ctrl = {
    ...
    async handleFiles(fileList) {
        ...
        if (!files.length)
            return;
        if (!window.XLSX) {
            App.UI.toast("Spreadsheet library not loaded.");
            return;
        }
    ...
    async run() {
        const keys = document.getElementById("inputKeys").value
            .split("\n").map(s => s.trim()).filter(Boolean);
        if (!keys.length) {
            App.UI.toast("Paste at least one key.");
            return;
        }
        if (!Object.keys(App.State.files).length) {
            App.UI.toast("Attach at least one source file.");
            return;
        }
        App.State.warnings = [];
    ...
    async rebuildClusters() {
        ...
        let big = false;
        Object.values(App.State.files).forEach(f => {
        ...
        if (big)
            App.State.warnings.push({ type: "warn", msg: "A source exceeds 100K rows — clustering may take a few seconds." });
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        btn.addEventListener("click", e => {
            ...
            if (idx != null) {
                setTimeout(() => {
                    ...
                    if (items[idx])
                        items[idx].scrollIntoView({ block: "start", behavior: "smooth" });
                }, 50);
            }
        });
    });
    ...
    document.getElementById("modalRegisterBtn").addEventListener("click", async () => {
        if (!modalWB) {
            App.UI.toast("Choose a workbook first.");
            return;
        }
        const id = document.getElementById("modalFileId").value.trim() || U.uid("file");
        if (App.State.profile.files.some(f => f.id === id)) {
            App.UI.toast("File id already exists.");
            return;
        }
        const sheet = document.getElementById("modalSheet").value;
        const hr = Number(document.getElementById("modalHeaderRow").value) || 1;
    ...
    slotGrid.addEventListener("change", async (e) => {
        ...
        if (t.dataset.slotSheet) {
            ...
            if (file)
                await App.Ctrl.loadIntoSlot(fd.id, file.filename, file.wb);
    ...
    slotGrid.addEventListener("keydown", e => {
        ...
        if (t.dataset.slotAddkey && e.key === "Enter") {
            ...
            if (val) {
                fd.key_columns = fd.key_columns || [];
                fd.key_columns.push(val);
            }
    ...
    slotGrid.addEventListener("click", async (e) => {
        ...
        if (t.dataset.slotReload) {
            ...
            if (file) {
                await App.Ctrl.loadIntoSlot(t.dataset.slotReload, file.filename, file.wb);
                App.UI.Sources.render();
            }
            return;
        }
    ...
    slotGrid.addEventListener("dragstart", e => {
        ...
        if (slotDragEl)
            slotDragEl.classList.add("dragging");
    });
    slotGrid.addEventListener("dragend", () => {
        if (slotDragEl)
            slotDragEl.classList.remove("dragging");
        slotDragEl = null;
    });
    slotGrid.addEventListener("dragover", e => {
        ...
        if (after)
            slotGrid.insertBefore(slotDragEl, after);
        else
            slotGrid.appendChild(slotDragEl);
    });
    ...
    schemaTable.addEventListener("click", e => {
        ...
        if (t.dataset.srcUp) {
            ...
            if (i > 0) {
                const s = f.sources.splice(i, 1)[0];
                f.sources.splice(i - 1, 0, s);
            }
            App.UI.Schema.render();
            markDirtyAndRefresh(false);
        }
        ...
        if (t.dataset.testRun) {
            ...
            if (!App.State.clusters.length) {
                out.textContent = "run resolution first";
                return;
            }
            const hit = App.ClusterEngine.lookup(key);
    ...
    schemaTable.addEventListener("change", e => {
        ...
        if (t.dataset.srcOverride) {
            ...
            if (t.checked)
                f.sources = App.Resolver.orderedSources(f); /* freeze current order */
            App.UI.Schema.render();
            markDirtyAndRefresh(false);
        }
        ...
        if (t.dataset.frename) {
            ...
            if (newName && newName !== oldName) {
                ...
                if (App.State.profile.canonical_schema.some(x => x.name === newName)) {
                    App.UI.toast("Field name already exists.");
                    t.value = oldName;
                    return;
                }
                f.name = newName;
    ...
    document.getElementById("rebuildBtn").addEventListener("click", async () => {
        if (!Object.keys(App.State.files).length) {
            App.UI.toast("Attach a file first.");
            return;
        }
        App.State.warnings = [];
    ...
    ["resultFilter", "filterBlanks", "filterResolved", "showClusterId"].forEach(id => document.getElementById(id).addEventListener("input", () => App.UI.Results.render()));
    document.getElementById("resultsTable").addEventListener("click", e => {
        const b = e.target.closest("[data-trail]");
        if (b) {
            App.UI.Results.toggleTrail(Number(b.dataset.trail), b);
            return;
        }
        /* drag may have just happened — skip sort if a column was reordered */
        if (App.State._justDragged) {
            App.State._justDragged = false;
            return;
        }
        ...
        if (th && App.State.results.length && th.dataset.col && th.dataset.col !== "trail") {
            ...
            App.State._sortKey = key;
            App.State._sortDir = dir;
    ...
    resultsTableEl.addEventListener("dragend", () => {
        if (thDragEl)
            thDragEl.classList.remove("col-dragging");
        resultsTableEl.querySelectorAll("th.col-drop-target").forEach(x => x.classList.remove("col-drop-target"));
        thDragEl = null;
    });
    ...
    document.getElementById("dlCsvBtn").addEventListener("click", () => App.Exporter.download("UPC_Concat_" + U.today() + ".csv", "text/csv", App.Exporter.toCSV()));
    ...
    document.getElementById("importProfileBtn").addEventListener("click", () => document.getElementById("profileFileInput").click());
    ...
    document.getElementById("discardBtn").addEventListener("click", () => {
        if (!App.Profile.isDirty()) {
            App.UI.toast("No unsaved changes.");
            return;
        }
