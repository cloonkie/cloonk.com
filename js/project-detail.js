...
(function () {
  ...
  var galleryHTML = galleryItems.length
    ? '<span class="detail-label-tag">Images</span>'
      ...
      + galleryItems.map(function(g){ return '<div class="detail-gallery__item" data-index="'+g.i+'"><img src="'+g.src+'" alt="'+p.title+' image '+g.i+'" loading="lazy" onerror="this.closest(\'.detail-gallery__item\').style.display=\'none\'"/><div class="detail-gallery__overlay"><span class="detail-gallery__zoom">↗ View</span></div></div>'; }).join('')
      + '</div>'
    : '';
  ...
  var nextLink = next
    ? '<a href="project.html?id=' + next.id + '" class="detail-breadcrumb__next"><span>Next</span><span class="detail-breadcrumb__sep">/</span><span class="detail-breadcrumb__next-title">' + next.title + '</span><span> →</span></a>'
  ...
  /* Scroll spy */
  var navBtns = {};
  root.querySelectorAll('.detail-section-nav__item').forEach(function(b) { navBtns[b.dataset.target] = b; });
  ...
  root.querySelectorAll('.detail-gallery__item').forEach(function(item,i){ item.addEventListener('click',function(){ openLB(i); }); });
